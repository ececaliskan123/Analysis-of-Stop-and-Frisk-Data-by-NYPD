...
#=============================
#2. Racial Compositions in Different Contexts
#=============================

rc = df1 %>% 
    group_by(race) %>% 
    summarize(avg_hit = 100 * mean(hitRate), freq = n()) %>%
...
ggplot(plot, aes(x = Precinct, y = Percentage, fill = race)) + 
  theme_grey() + 
  geom_bar(position = "dodge", stat = "identity") + 
  facet_wrap(~)
  xlab("Precinct") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(title='Racial Compositions in Different Contexts')

ggplot(rc, aes(y=value, x=race, color=race, fill=race)) + 
