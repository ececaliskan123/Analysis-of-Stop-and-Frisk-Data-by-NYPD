# ******************************************************* 
# This code combines the annual raw data into one and creates all relevant variables.
# Only stops concerning "Criminal Possession of a weapon" are concerned and only
# the relevant covariates which have also been used by Goel et al (2016) are kept.
#
# Outline: 1) Subsetting Data, 2) Transforming coordinates,
# 3) Calculate local hit rate
# *******************************************************

#### LOAD YEARLY DATASETS & SUBSET RELEVANT ROWS & COLUMNS
...
AllHRs = lapply(years, function(x){hitRateCalculation(x,100)})  # calculates hitRates for all yearas
names(AllHRs) = years
...
df$hitRate = NA     # create empty colum
for(i in years){    # fill column in loop
